...
{
public:
    LeftArmController() : node_(rclcpp::Node::make_shared("moveit2_left_control"))
    {
        ...
        subscriber_ = node_->create_subscription<geometry_msgs::msg::Pose>(
            "/s17_common/moveit2_control/l_arm/pose",
            10,
            std::bind(&LeftArmController::poseCallback, this, std::placeholders::_1));

        // サービスサーバーの設定
        service_ = node_->create_service<s17_msgs::srv::SetPosition>(
            "/s17_common/l_arm/set_position",
            std::bind(&LeftArmController::setPositionCallback, this, std::placeholders::_1, std::placeholders::_2));
